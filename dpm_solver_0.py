...
class NoiseScheduleFlow:
    ...
    def inverse_lambda(lamb):
        ...
        #return torch.exp(-lamb) / (torch.exp(-lamb) + 1)
        return 1-torch.sigmoid(lamb)
...
class DPM_Solver:
    ...
    def update_progress(self, step, total_steps):
        ...
        """
        if hasattr(self, "progress_fn"): #检查是否有progress_fn
            try:
    ...
    def singlestep_dpm_solver_second_update(
        ...
        s1 = min(ns.inverse_lambda(lambda_s1), (s+t)/2)
        print("s,s1,t",s,s1,t,lambda_s1)
    ...
    def sample(
        ...
        t_T = self.noise_schedule.T - 1e-3 if t_start is None else t_start
        assert (
            t_0 > 0 and t_T > 0
        ), "Time range needs to be greater than 0. For discrete-time DPMs, it needs to be in [1 / N, 1], where N is the length of betas array"
        if return_intermediate:
