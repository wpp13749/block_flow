...
from dpm_solver_0 import NoiseScheduleFlow, model_wrapper, DPM_Solver
class BaseFlow():
  ...
  @torch.no_grad()
  def sample_ode_generative_y(self, z1=None, N=None, use_tqdm=True, solver = 'euler',y=None):
    assert solver in ['euler', 'heun']
    tq = tqdm if use_tqdm else lambda x: x
    if N is None:
      N = self.N    
    ...
    for i in tq(reversed(range(1,N+1))):
      ...
      if len(z1.shape) == 2:
        ...
        vt = self.model(z, t.squeeze(),y)
        if solver == 'heun' and i > 1:
          z_next = z.detach().clone() + vt * dt
          vt_next = self.model(z_next, t_next.squeeze(),y)
          vt = (vt + vt_next) / 2
        x0hat = z - vt * t.view(-1,1,1,1)
        x0hat_list.append(x0hat)
  ...
  def sample_ode_generative_bbox(self, z1=None, N=None, use_tqdm=True, solver = 'RK45', eps = 1e-3, rtol=1e-5, atol=1e-5,):
    ...
    return result, nfe
  def sample_ode_generative_bbox_y(self, z1=None, N=None, use_tqdm=True, solver = 'RK45', eps = 1e-3, rtol=1e-5, atol=1e-5,y=None):
    dshape = z1.shape
    device = z1.device
    def ode_func(t, x):
      x = torch.from_numpy(x.reshape(dshape)).to(device).type(torch.float32)
      vec_t = torch.ones(dshape[0], device=x.device) * t
      vt = self.model(x, vec_t,y)
      vt = vt.detach().cpu().numpy().reshape(-1)
      return vt
    solution = integrate.solve_ivp(ode_func, (1, eps), z1.detach().cpu().numpy().reshape(-1), method=solver, rtol = rtol, atol = atol)
    nfe = solution.nfev
    result = torch.from_numpy(solution.y[:,-1].reshape(dshape))
    return result, nfe
  def sample_dmp_solver(self,z1=None):
    ...
    model_fn_continuous,
    noise_schedule,
    algorithm_type="dpmsolver++",  # "dpmsolver"
    )
    result = dpm_solver.inverse(
    z1, 
    steps=40, 
    order=3,
    skip_type="time_uniform_flow",  # 'logSNR' 'time_uniform' 'time_quadratic' "time_uniform_flow"
    method="multistep",  # 'singlestep' "multistep" "adaptive"
    lower_order_final=False,
    denoise_to_zero=True,
    solver_type="dpmsolver",  # `dpmsolver` or `taylor`
    atol=0.0078, 
    rtol=0.05, 
    flow_shift=3.0
    )  #注意事项：如果你模型是t=0的时候是噪声，t=1的时候是原图，那么把zt = dpm_solver.sample改为dpm_solver.inverse
...
class RectifiedFlow(BaseFlow):
  def get_train_tuple(self, z0=None, z1=None, t = None, eps = 1e-5):
    ...
    return z_t, t, target
  def get_train_tuple_y(self, z0=None, z1=None, t = None,mu=None,logvar=None,eps = 1e-5):
    if t is None:
      t = torch.rand((z1.shape[0], 1), device=self.device)
    t = t * (1 - eps) + eps
    if len(z1.shape) == 2:
