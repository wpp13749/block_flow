...
def train_rectified_flow(rectified_flow, forward_model, optimizer, train_data1, train_data2, batchsize, iterations):
    ...
    for i in range(iterations+1):
        ...
        if independent:
            ...
            y=i%2*torch.ones(batchsize,1).to(device)
            out = forward_model(None, torch.ones(batchsize, 1, device = device),y=y)
            mu = out[:, :2]
            logvar = out[:, 2:]
        ...
        loss_prior = torch.norm(logvar, p=2)/ (logvar.shape[1]) if not independent else 0
        ...
        if i % 100 == 0:
            ...
            if i % 1000 == 0:
                with torch.no_grad():
                    ...
                    if independent:
                        ...
                        out1 = forward_model(None, torch.ones(batchsize, 1, device = device),y=torch.zeros(batchsize,1).to(device))
                        out2 = forward_model(None, torch.ones(batchsize, 1, device = device),y=torch.ones(batchsize,1).to(device))
                    ...
                    if wide_prior:
                        z[:, 0] = z[:, 0] * 6
                    out_y = forward_model(None, torch.ones(2048, 1, device = device),y=torch.randint(0, 2, (2048,1), device=device))
                    mu_y=out_y[:, :2]
                    logvar_y = out_y[:, 2:]
                    z_y=mu_y+torch.randn_like(mu_y)* torch.exp(logvar_y/2)    
                    traj_y, _ = rectified_flow.sample_ode_generative(z1=z_y, N=N)
                    traj_forward_x1 = rectified_flow.sample_ode(z0=x1)
                    traj_forward_x2 = rectified_flow.sample_ode(z0=x2)
                    

                    traj_particles_y = torch.stack(traj_y).cpu().numpy()
                    ...
                    plt.yticks([])
                    for j in range(30):
                        plt.plot(traj_particles_y[:, j, 0], traj_particles_y[:, j, 1], color='blue')
                     
                    plt.scatter(traj_y[-1].cpu().numpy()[:, 0], traj_y[-1].cpu().numpy()[:, 1], alpha=0.15, color="blue", s = DOT_SIZE)

                    # plt.title('Transport Trajectory')
                    plt.tight_layout()
                    plt.savefig(os.path.join(dir, f'traj_y{i}.jpg'), dpi=300)
            ...
            if i % 10000 == 0:
                ...
                with torch.no_grad():
                    ...
                    y_test=torch.cat([torch.zeros(10000,1), torch.ones(10000,1)], dim=0).to(device)
                    if independent:
                        z = torch.randn_like(x_test)
                        if wide_prior:
                            z[:, 0] = z[:, 0] * 6
                    else:
                        out = forward_model(None, torch.ones(x_test.shape[0], 1, device = device),y=y_test)
                        mu = out[:, :2]
                        logvar = out[:, 2:]
                        z = mu + torch.randn_like(mu) * torch.exp(logvar/2)
                    for j in range(10):
...
forward_model = MLP(input_dim=3,output_dim = 4).to(device)
