...
# in this part,we set x_droupout. it doesn't change the netxork too much
...
class SongUNet(torch.nn.Module):
    ...
    def forward(self, x, noise_labels, class_labels = None, augment_labels=None,x_dropout=None):
        # Mapping.
        ...
        skips = []
        if x is not None:
            x=x*(x_dropout is None or (torch.rand((), device=noise_labels.device) >= x_dropout)) 
        else:
            x=torch.zeros(emb.shape[0], self.in_channels, self.img_resolution, self.img_resolution).to(emb.device)
        aux = x 
