    ...
    with torch.no_grad():
        ...
        for ep in tqdm(range(epoch)):
            ...
            if arg.encoder is not None:
                ...
                # z, _, _ = forward_model(x, noise = noise,y=y)
            else:
                z = noise
            ...
            if arg.solver in ['euler', 'heun']:
                traj_uncond, traj_uncond_x0 = rectified_flow.sample_ode_generative_y(z1=z, N=arg.N, use_tqdm = False, solver = arg.solver,y=y)
                x0 = traj_uncond[-1]
