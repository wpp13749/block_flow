...
def train_rectified_flow(rank, rectified_flow, forward_model, optimizer, data_loader, iterations, device, start_iter, warmup_steps, dir, learning_rate, independent,
    ...
    for i in tqdm_(range(start_iter, iterations+1)):
        ...
        if independent:
            ...
            z, mu, logvar = forward_model(x, torch.ones((x.shape[0]), device=device),y=y,x_dropout=0.5)

        z_t, t, target = rectified_flow.get_train_tuple(z0=x, z1=z)
        
        
        # Learn reverse model
        pred = rectified_flow.model(z_t, t.squeeze(),y)


        loss_fm = torch.mean((target - pred)**2)
        loss_fm = loss_fm.mean()
        loss_prior_y = get_kl(mu, logvar)
        loss = loss_fm + weight_prior * (loss_prior_y)#
        ...
        if i % 100 == 0 and rank == 0:
            loss_fm_ratio = (loss_fm.item() / loss.item()) if loss.item() != 0 else 0.0
            
            print(f"Iteration {i}: loss {loss.item()}, loss_fm {loss_fm.item()}, loss_prior_y {loss_prior_y.item()}, loss_fm_ratio {loss_fm_ratio:.4f}")
            ...
            with open(os.path.join(dir, 'log.txt'), 'a') as f:
                f.write(f"Iteration {i}: loss {loss.item():.8f}, loss_fm {loss_fm.item():.8f}, loss_prior_y {loss_prior_y.item():.8f}, loss_fm_ratio {loss_fm_ratio:.4f}, lr {optimizer.param_groups[0]['lr']:.4f} \n")
        ...
        if i % 5000 == 1 and rank == 0:
            ...
            with torch.no_grad():
                if independent:
                    ...
                    # noise = torch.randn(y.shape[0], 3, 32, 32, device=y.device)
