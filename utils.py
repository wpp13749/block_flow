...
def get_kl(mu, logvar):
    ...
    return loss_prior
def get_kl_y(mu, logvar, mu_y, logvar_y):
    """
    ...
    # KL散度计算
    kl = 0.5 * torch.sum(
        (logvar_y - logvar) +  # 对数方差项
        (torch.exp(logvar) / torch.exp(logvar_y)) +  # 方差比
        ((mu - mu_y).pow(2) / torch.exp(logvar_y)) -  # 均值差平方除以方差
        1,  # 常数项
        dim=[1, 2, 3]
    )
    ...
    return loss_prior
def get_kl_y_mu_y_1(mu, logvar, mu_y, logvar_y):
    """
    计算N(mu, var)和N(mu_y, exp(logvar_y))之间的KL散度，并除以数据维度。
