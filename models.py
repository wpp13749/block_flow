...
class UNetEncoder(nn.Module):
    ...
    def forward(self, x=None, t = None,noise = None,y=None,x_dropout=0.):
        if t is None:
            if x is None:
        ...
        if noise is None:
            if x is None:
                ...
                #noise = torch.randn(y.shape[0], 1, 28, 28, device=y.device) for MNIST
            else:
                noise = torch.randn_like(x)
        z, mu, logvar = self.encode(x, t, noise , y=y,x_dropout= x_dropout)
        return z, mu, logvar
    def encode(self, x, t , noise, y=None, x_dropout=0.):
        if y is None:
            mu_logvar = self.encoder(x, t, x_dropout= x_dropout)
        else:
            mu_logvar = self.encoder(x, t, y, x_dropout= x_dropout)
        mu, logvar = mu_logvar[:, :self.input_nc], mu_logvar[:, self.input_nc:]
